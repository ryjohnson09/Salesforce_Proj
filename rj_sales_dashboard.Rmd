...
calls_account <- rj_acct %>% 
  left_join(all_calls_test, by = "account_id") %>% 
  select(account_name, activity_date) %>% 
  # Calculate days from last call
  mutate(days_last_call = as.double(difftime(
    ymd(today),
    ymd(activity_date),
    units = "days"))) %>% 
...
```{r}
sunburst_tree <- rj_sunburst_table %>% 
  select(account_name, opp_name_days, opp_amount) %>% 
  d3_nest(value_cols = "opp_amount")

my_colors <- list(range = c(calls_account_color$acct_color, opps_color$opp_color),
                  domain = c(calls_account_color$account_name, opps_color$opp_name_days))
